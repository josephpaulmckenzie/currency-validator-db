...
async function getTextDetections(imageData: string | Buffer) {
  try {
    ...
    if (error instanceof Error) {
      throw new Error('Error processing image: ' + (error.message || error));
    } else {
...
function getAdditionalDetails(
  denomination: Interfaces.DenominationDetail[],
  serialNumber: string
): Interfaces.MatchedDetail {
  try {
    if (denomination) {
      const matchedDetail = denomination.find(detail => {
        // Check if the pattern is a RegExp or a string and create the appropriate RegExp object
        const regex =
          typeof detail.pattern === 'string'
            ? new RegExp(detail.pattern)
            : detail.pattern;
  ...
  try {
    ...
    for (const text of textDetections) {
      ...
      if (detectedText) {
        ...
        // Check against note validator patterns
...
const serialNumberPatterns: Interfaces.RegExValidators = {
  ...
  isLadderSerialPattern:
    /^(?:\d{8}|01234567|12345678|23456789|87654321|98765432)$/,
  // Examples: 'A12345678D,DL98765432D'

  // Checks to see if a serial number contains all the same digits.
  isASolidNumberSerialPattern:
    /^(?:[A-Q]?[A-L](?:\s?(\d)\1{7})(?:\s?[A-Q])|(\d)\2{7})$/,
